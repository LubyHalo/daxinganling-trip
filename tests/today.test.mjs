...
await check('出发日：今天视图锁定 DAY 1，并给出航班与取车倒推', () => {
  ...
  assert.match(t, /07:30/, '应给出建议出发时间（10:00 起飞的 2.5 小时提前量）');
  assert.match(t, /已过——确认已经在路上/, '08:00 已过 07:30，应提示而不是继续显示"建议出发"');
  assert.ok(!/06:00/.test(t), '不应再出现按 4 小时假设算出的 06:00（回归测试）');
...
/* ---- 出发日清晨（2026-09-19 06:00，还没到建议出发时间）---- */
const vtEarly = await bootAt('2026-09-19T06:00:00');
await check('出发日清晨：给出 07:30 出发建议与提前量说明', () => {
  const t = text(vtEarly.window);
  assert.match(t, /距 10:00 起飞还有 4 小时 0 分钟/);
  assert.match(t, /建议 <b>07:30<\/b>|建议 07:30/, '应建议 07:30 前从舟山出发');
  assert.match(t, /家到舟山机场约 20 分钟/, '应显示提前量的说明文字（数据驱动，不是写死的）');
  assert.ok(!/06:00/.test(t), '不应再出现 06:00（回归测试）');
});
